...
Ce site utilise [l'optimisation des images d'Astro](https://docs.astro.build/guides/assets/). Dans le dépôt, les images ne sont pas traitées et sont conservées dans leur format d'origine, mais lors du _build_, elles sont redimensionnées et converties au format WebP. Certains sites vont plus loin en proposant également le format <abbr>AVIF</abbr>, plus moderne, mais je n'ai pas trouvé de réglage offrant un compromis satisfaisant entre le poids et la qualité.

### Images de substitution

J'ai repris la technique développée par mon amie Erika pour son [site personnel](https://erika.florist/) et consistant à créer, pour chaque image, une image de substition (ou _placeholder_) à la résolution très faible afin de l'encoder dans la page <abbr>HTML</abbr>, plus précisément dans le `background` de `<img>`. Ainsi, avant que la véritable image ne soit chargée, le navigateur affiche un dégradé abstrait, bien plus esthétique qu'un simple fond blanc.
...
J'utilise la librairie [Medium Zoom](https://medium-zoom.francoischalifour.com/) pour afficher une version en haute résolution de l'image lors du clic. La _lightbox_ utilise la couleur de fond de la palette pour une expérience plus immersive.

## Blocs et mentions

### Galeries

Une galerie est une succession d'images sur la même ligne dans une _flexbox_. Pour optimiser le poids de la page, les images d'une galerie peuvent être deux ou trois fois plus petites qu'une image normale. Le défaut de cette galerie est que chaque élément de la _flexbox_ fait la même taille, ce qui signifie que les images doivent toutes faire la même taille pour éviter les espaces vides. Une astuce pour redimensionner les éléments de la _flexbox_ dynamiquement en fonction de la taille des images serait bienvenue !

<sc>Exemple</sc> : <BlogRef slug="2021/destiny-introduction-analyse" titleOnly />
...
<sc>Exemple</sc> : <BlogRef slug="2021/creer-un-site-avec-hugo" titleOnly />
...
[The Movie Database](https://www.themoviedb.org) (<abbr>TMDB</abbr>) est une alternative gratuite à [<abbr>IMDB</abbr>](https://www.imdb.com/) et son <abbr>API</abbr> payante. C'est un service utilisé par de nombreux sites dont [Letterboxd](https://letterboxd.com/). Il est presque toujours possible d'obtenir un trailer et le casting, mais les images ont tendance à manquer sur les films peu populaires.
...
Les _pop-ups_ affichent des informations sur un jeu, un film ou une personnalité (voir [<sc>IGDB</sc>](#igdb) et [<sc>TMDB</sc>](#tmdb)). Dans le cas d'une œuvre, la _pop_up_ contient le titre, l'affiche, les auteurs, l'année de parution et jusqu'à trois images. Une _pop-up_ s'ouvre en cliquant sur un texte dont le lien est d'une apparence plus discrète que les liens externes. Selon la palette, le lien peut être coloré ou en pointillé.

{/* prettier-ignore */}
<sc>Exemple</sc> : Le film <Popup movie="558582">_First Cow_</Popup>, le jeu <Popup game="superbrothers-sword-sworcery-ep">_Sword & Sorcery_</Popup>.
...
Vous trouverez plusieurs utilisations du bloc dans les notes de base de page de <BlogRef slug="2023/small-caps" titleOnly/>.

### Mentions d'article

Ces mentions utilisent les couleurs et la police de la palette de l'article. Les mentions peuvent être une référence complète (<BlogRef slug="2023/historique-rpg-maker"/>) ou indiquer seulement le titre (<BlogRef slug="2023/historique-rpg-maker" titleOnly/>).

## Autres fonctionnalités

- La page de recherche ([`/search`](/search)), dont la logique est tirée du [site de Goulven](https://goulven-clech.dev/), permet de retrouver les entrées des _content collections_ ainsi que quelques autres pages ajoutées manuellement.
- L'interface de navigation du blog ([`/categories`](/categories), [`/tags`](/tags), etc.) s'adapte au mode clair ou sombre de l'appareil du visiteur. En mode sombre, la liste des articles est enveloppée d'une lueur afin de préparer le visiteur qui pourrait être ébloui en cliquant sur un article dont la palette est claire.
- Les pages de jeu, accessibles depuis [`/games`](/games), contiennent une porte dérobée. Un double-clic sur le titre renvoie vers la page Notion qui sert à en générer le contenu. Je m'inspire de Gwern qui rend la source accessible [en ajoutant `.page` à une <abbr>URL</abbr>](https://gwern.net/about#source), mais ma variante est plus pratique sur mobile.
- Le site utilise les [_View Transitions_](https://docs.astro.build/en/guides/view-transitions/) d'Astro, affichant un fondu entre les pages et parfois une transition plus travaillée. Cela nécessite d'être particulièrement vigilant quant à la logique d'exécution du code JavaScript au sein des pages.
...
- **Améliorer le design** :
  ...
  - <sc>Layouts</sc> : Les transformer en un enum, comme les palettes, pour un code plus sûr et
    clair.
  - <sc>Pop-ups</sc> : Retirer les espaces avant et après les liens ; rendre la gestion des couleurs
    plus flexible.
- **Pop-ups** : ajouter les données de [`/games`](/games) au _pop-ups_ de jeu.
...
- **Ajouter une galerie des films** que j'ai mentionnés sur ce site ou aimés. C'est un problème épineux, car [Letterboxd](https://letterboxd.com/) a refusé ma demande d'accès à l'<abbr>API</abbr>.
- **Ajouter un bloc GitHub** pour mentionner un dépôt ou un utilisateur.
- **Ajouter l'historique git** sur la page [`/changelog`](/changelog).
