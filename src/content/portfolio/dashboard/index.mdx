...
J'ai développé une application web avec [Next.js](https://nextjs.org/), [SWR](https://swr.vercel.app/) et l'[API de Notion](https://developers.notion.com/), permettant d'afficher une série de rapports élégants dans des tableaux mis à jour en direct. Le serveur web agit comme un intermédiaire filtrant les données sensibles du Notion de l'agence, notamment les coordonnées de nos auteurs. Ainsi, il est entièrement sûr de délivrer la page web au client pour lui partager l'avancement.
...
L'API de Notion est particulièrement lente à produire une réponse. Ce n'est pas forcément un problème dans des cas de figure classiques, mais ici, j'ai parfois besoin de charger plus de 500 cours, qui sont autant d'entrées dans la base de données, pour calculer des statistiques globales par exemple. Le problème, c'est que charger autant d'entrées peut parfois prendre plus de 5 secondes, ce qui altère grandement l'expérience de navigation, et me fait de surcroït dépasser la limite maximale de durée pour une fonction d'API sur [Vercel](https://vercel.com), l'hébergeur du site.
